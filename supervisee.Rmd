...
```{r}
library(MASS)
res_AIC <- glm(New ~ ., family = binomial , data=data_num.train)
```

```{r}
pred_AIC = predict(res_AIC, newdata=data_num.test , type="response")
mean(Dtest_s == 1*(pred_AIC>0.5))
table(Dtest_s,1*(pred_AIC>0.5))
...
```{r}
library("glmnet")
res_Lasso <- glmnet(as.matrix(data_num.train[,-1]), data_num.train$New, family='binomial')
summary(res_Lasso)
```

```{r}
cvLasso <- cv.glmnet(as.matrix(data_num.train[,-1]),data_num.train$New,family="binomial",type.measure = "class")
plot(cvLasso)
...
```{r}
library(pROC)
pred_logit_lasso=predict(cvLasso, newx = as.matrix(data_num.test[,-1]), s = 'lambda.min', type = "response")
...
```{r}
library("gbm")
fit.adaboost = gbm(New ~., data_num.train, distribution = "adaboost")
